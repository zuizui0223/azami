...
def test_priority_was_fixed_without_complete_diagnostic_or_trait_outcomes():
    p = json.loads(PRIORITY.read_text(encoding="utf-8"))
...
def test_direct_process_priority_matches_frozen_hypotheses():
    roles = json.loads(PRIORITY.read_text(encoding="utf-8"))["candidate_roles_before_diagnostics"]
...
def test_redundancy_priority_is_predeclared_not_p_value_selected():
    ...
    assert "pr_month" in p["candidate_roles_before_diagnostics"]["BIO12"]["redundancy_preference"] or "observation-month precipitation" in p["candidate_roles_before_diagnostics"]["BIO12"]["redundancy_preference"]
    assert "non-redundant" in rules
