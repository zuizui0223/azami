...
def aggregate(report_paths, batch_index: int, *, root: Path = ROOT):
    ...
    result = {
        ...
        "batch_can_authorize_stopping": False,
        "historical_wilson_decisions_are_descriptive_only": True,
        "precision_amendment": "analysis/v3/calibration_precision_amendment.json",
        "qualification_boundary": "A complete batch is inventory, not admission. The amended cumulative evaluator requires returned outer arrays and independent-outer, time-uniform uncertainty. Historical pooled Wilson output never qualifies; realized-design calibration remains required.",
