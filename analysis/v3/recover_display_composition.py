...
def recover(heads, out, expected_sha=HEAD_SHA, expected_rows=1255791):
    ...
    try:
        with sqlite3.connect(out / "display_composition.sqlite") as db, heads.open(encoding="utf-8-sig", newline="") as handle:
            db.execute("CREATE TABLE heads (source_row INTEGER PRIMARY KEY, head_id TEXT UNIQUE, photo_id TEXT, obs_id TEXT, colour_status TEXT, " + ",".join(f"{field} REAL" for field in FIELDS) + ",visible_eligible INTEGER,composition_eligible INTEGER,composition_qc_conflict INTEGER,raw_fields_json TEXT)")
            ...
            for count, row in enumerate(reader, 1):
                ...
                batch.append((count, row["annotation_unit_id"], row["photo_id"], row["obs_id"], row["colour_status"], *values, visible, composition, conflict, json.dumps({f: row[f] for f in FIELDS}, sort_keys=True)))
                if len(batch) == 20000:
            ...
            db.execute("CREATE TABLE photo_values AS SELECT obs_id,photo_id,COUNT(*) AS n_heads_total,SUM(visible_eligible) AS n_visible_heads,SUM(composition_eligible) AS n_composition_heads," + ",".join(means) + " FROM heads GROUP BY obs_id,photo_id")
            db.execute("CREATE TABLE observation_values AS SELECT obs_id,COUNT(*) AS n_photos_total,SUM(n_heads_total) AS n_heads_total,SUM(n_visible_heads>0) AS n_visible_photos,SUM(n_composition_heads>0) AS n_composition_photos," + ",".join(f"AVG({f}) AS {f}" for f in FIELDS) + " FROM photo_values GROUP BY obs_id")
            scalar = lambda sql: db.execute(sql).fetchone()[0]
            fields = {}
            for endpoint, field in zip(ENDPOINTS, FIELDS):
                fields[endpoint] = {"source_field": field, "finite_head_values": scalar(f"SELECT COUNT(*) FROM heads WHERE {field} IS NOT NULL"), "finite_colour_qc_usable_heads": scalar(f"SELECT COUNT(*) FROM heads WHERE colour_status='usable' AND {field} IS NOT NULL"), "recovered_photo_values": scalar(f"SELECT COUNT(*) FROM photo_values WHERE {field} IS NOT NULL"), "recovered_observation_values": scalar(f"SELECT COUNT(*) FROM observation_values WHERE {field} IS NOT NULL")}
