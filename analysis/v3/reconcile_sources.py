        ...
        with zipped.open(raw_name) as binary:
            ...
            for line, payload in enumerate(handle, 1):
                ...
                if len(observations) == 5000:
                    ...
                    links.clear()
                if line % 50000 == 0:
                    print(json.dumps({"artifact": spec["artifact_id"], "raw_observations_read": line}), flush=True)
        db.executemany("INSERT INTO api_observations VALUES (?,?,?,?,?)", observations)
        db.executemany("INSERT INTO api_photo_links VALUES (?,?,?,?,?)", links)
        ...
        with sqlite3.connect(out / "source_reconciliation.sqlite") as db:
            ...
            for spec in manifest["archives"]:
                if spec["role"] == "raw_metadata_chunk":
                    ...
                    db.commit()
                    print(json.dumps({"artifact": spec["artifact_id"], "stage": "source_links_reconciled"}), flush=True)
            with metadata.open(encoding="utf-8-sig", newline="") as handle:
                ...
                db.executemany("INSERT INTO merged_photo_rows VALUES (?,?,?,?,?)", batch)
            db.executescript("""
                CREATE INDEX original_version ON original_photo_rows(origin, payload_sha256);
                CREATE INDEX merged_version ON merged_photo_rows(origin, payload_sha256);
                ...
                CREATE TABLE source_links AS
                    SELECT obs_id, photo_id FROM original_photo_rows WHERE obs_id<>'' AND photo_id<>''
                    UNION SELECT obs_id, photo_id FROM api_photo_links WHERE obs_id<>'' AND photo_id<>'';
                CREATE UNIQUE INDEX source_link ON source_links(obs_id, photo_id);
                CREATE INDEX source_photo ON source_links(photo_id);
                CREATE TABLE source_observations AS
                    SELECT obs_id FROM original_photo_rows WHERE obs_id<>''
                    UNION SELECT obs_id FROM api_observations WHERE obs_id<>'';
                CREATE UNIQUE INDEX source_observation ON source_observations(obs_id);
            """)
            scalar = lambda sql: db.execute(sql).fetchone()[0]
            if scalar("SELECT COUNT(*) FROM merged_photo_rows m WHERE NOT EXISTS (SELECT 1 FROM original_photo_rows o WHERE o.origin=m.origin AND o.payload_sha256=m.payload_sha256)"):
                raise ValueError("Merged rows do not match exact original source versions")
            counts = {
            ...
            if counts["merged_metadata_rows"] != contract["source"]["expected_photo_rows"] or counts["merged_metadata_observations"] != contract["source"]["expected_observations_with_photos"]:
                raise ValueError("Merged counts differ from the pinned snapshot")
            for chunk in chunks:
                origin = str(chunk["artifact_id"])
                chunk["metadata_observations"] = db.execute("SELECT COUNT(DISTINCT obs_id) FROM original_photo_rows WHERE origin=? AND obs_id<>''", (origin,)).fetchone()[0]
                chunk["seen_minus_metadata_observations"] = chunk["collection"]["observations_seen"] - chunk["metadata_observations"]
            for previous, current in zip(chunks, chunks[1:]):
                if previous["collection"]["last_obs_id"] != current["collection"]["requested_start_obs_id"]:
                    raise ValueError("Collection chunk ID boundaries do not join")
        report = {
            "status": "ARCHIVED_SOURCE_LINKS_RECONCILED_WITH_EXPLICIT_COVERAGE_GAPS",
            "counts": counts, "chunks": chunks,
            ...
            "image_bytes_verified": False, "ecological_models_executed": False,
            "limits": [
                "Archive recovery is not evidence of image-byte availability or new trait accuracy.",
                "Unique links are a derived view; all original metadata and API record locators remain in separate tables and exact archives.",
                "A photo linked to several observations is not an independent replicate and may have conflicting source assignments.",
                "Missing raw API archives prevent complete reconstruction of collection-time losses.",
                "The query required photos; its changing API population and records outside that query are not a closed census.",
            ],
        }
...
def main():
    ...
    result = reconcile(args.archives, args.metadata, args.out_dir,
