...
def audit_history(archive, metadata, out, spec, contract):
    ...
    try:
        with sqlite3.connect(out / "historical_processing.sqlite") as db, zipfile.ZipFile(archive) as zipped:
            ...
            db.executescript("CREATE UNIQUE INDEX head_detection ON heads(queue_id, det_index); CREATE INDEX head_obs ON heads(obs_id); CREATE INDEX head_photo ON heads(photo_id);")
