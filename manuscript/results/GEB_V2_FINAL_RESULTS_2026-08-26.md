...
## Cross-scale within- versus among-taxon alignment

After the completed GEB v2 artifact was frozen, a separate analysis contract was committed before inspecting cross-scale outcomes. The measured 18 endpoints correspond to 17 inferential units after circular hue sine/cosine are treated jointly. The among-taxon analysis used taxon-median traits and taxon-median BIO1, BIO4, BIO12 and BIO15, required at least five usable measurements of an endpoint per taxon in the primary scope, and tested standardized taxon-level associations with 10,000 taxon-label permutations. BH correction was separate for primary and candidate tiers. A minimum-two-measurement scope was frozen as a sensitivity analysis.

The primary >=5 scope fitted 68 endpoint-predictor models. Four passed the among-taxon FDR rule, all in the established primary tier: orientation-BIO12 (β = +0.304359, q = 0.006399) and joint corolla hue with BIO1 (magnitude = 0.303788, q = 0.010399), BIO4 (0.334766, q = 0.006399) and BIO15 (0.410191, q = 0.003200). The >=2 sensitivity again returned four FDR rows; orientation-BIO12 plus hue-BIO1 and hue-BIO15 were stable across both replication thresholds, while hue-BIO4 was supported only at >=5 and hue-BIO12 only at >=2.

Crossing the primary among-taxon screen with the registry-driven within-taxon screen classified the 68 endpoint-predictor rows as 3 `both_scales`, 8 `within_only`, 1 `among_only` and 56 `neither`. The established orientation signal illustrates scale specificity: orientation-BIO1 is supported within taxa but not among taxa, whereas orientation-BIO12 is supported among taxa but not within taxa. Joint hue contains the clearest partial overlap across scales, although camera-colour calibration remains an open validation requirement.

Crucially, none of the nine candidate endpoints passed among-taxon FDR. The three candidate rows retained in the GEB v2 evidence atlas are all `within_only`: involucre length/width-BIO15, apical taper-BIO12 and bract projection p95-BIO15. Their unsupported among-taxon coefficient signs are not interpreted. The result is that these finer involucral associations currently describe geographic phenotype-environment structure within source-assigned taxa rather than systematic climate sorting among taxa.

A second frozen analysis extended the legacy environmental-PCA low/high-trait permutation test to the complete measured trait universe. Requiring at least five usable measurements for every measured inferential endpoint produced a common set of 44 taxa. None of the 16 linear endpoints passed BH correction for environmental centroid distance or Bhattacharyya overlap; joint circular hue retained an association (multivariate R² = 0.175622, permutation p = 0.015198). At the predeclared >=2 sensitivity, 78 common taxa were retained; orientation was the only linear endpoint passing both sorting metrics (centroid distance = 1.294203, q = 0.046195; Bhattacharyya overlap = 0.627559, q = 0.031497), and joint hue was also supported (R² = 0.145357, p = 0.000800). No candidate linear endpoint passed this expanded among-taxon sorting analysis at either threshold.

Taxon-label permutation is not phylogenetic correction, and agreement or disagreement across the two scales is not evidence of a common causal mechanism. These analyses instead show that the environmental organization of capitulum phenotype changes with biological scale.

## Submission-facing synthesis

The final GEB v2 result is not a universal climate-associated thistle morphology. Public biodiversity photographs recover a multidimensional, continuous capitulum phenotype landscape with substantial repeated variation below taxon means. Environmental structure differs among phenotype modules and analytical scales, and the explicit within-versus-among comparison shows only partial cross-scale overlap. Orientation changes environmental axis between scales, visible hue contains the clearest cross-scale sorting but remains colour-calibration sensitive, and the new fine involucral signals are currently restricted to within-taxon geographic structure rather than supported among-taxon climate sorting.

The v2 contribution is therefore threefold: it extends global public-image phenomics beyond species means and coarse categories to an explicit 27-endpoint continuous contract; it resolves phenotype-environment structure separately within and among taxa rather than conflating them; and it makes robustness part of the result by assigning evidence grades without confusing pattern detection with causal or functional inference.

## Claim boundary

Do not use this run or the subsequent cross-scale extension to claim phenotypic plasticity, local adaptation, pollinator causation, antagonist defence, botanical spine length, phyllary function, secretion function, evolutionary rate or phylogenetically independent among-taxon effects. `visible_floret_fraction` was not executed and cannot support a floral-display inference in this version. Candidate involucre/armature endpoints remain image-derived continuous phenotypes until independent botanical reference validation supports more specific naming.
