...
## Executed independent source-image sample
...
Workflow run `29900511784` selected a deterministic 1,100-image reserve. Of these, 1,099 downloaded and decoded successfully. The first 1,000 successful rows in the pre-detector taxon/spatial round-robin order were frozen as the audit. The executed packet contains:

- 1,000 source images;
- 323 species;
- 85 ten-degree spatial blocks;
- 250 preassigned double-labelled images;
- zero overlapping photo IDs with the development source set;
- zero overlapping observation IDs with the development source set.

The reserve design handles stale media URLs without replacing failures according to detector output or human trait visibility. Ninety-nine successfully downloaded reserve images remain unused.

The workflow produced three strictly separated artifacts:

1. blinded annotation packet `8521924881`, digest `sha256:1ebc0dac5316fa6b3c394f014f055facef6fa15c6b52c437932fdf62e60ea35c`;
2. private sampling key `8521925441`, digest `sha256:4c39fe51b155d0eb09038e470088ababce4367f740a476dc38127d801d2e0087`;
3. hidden detector predictions `8521926057`, digest `sha256:94e7582a795d88251bb32534b0a60ed8b66459f4eb8bbe133dafb5f3adff7642`.
...
The local application `analysis/detector_audit_annotation_app.py` displays only blinded source images. Annotator 1 completes all 1,000 images; annotator 2 completes the preassigned 250-image subset.
...
1. The leakage-free packet and hidden predictions were generated by `.github/workflows/ch1-independent-detector-audit-packet.yml`.
2. Annotate the downloaded blinded packet twice.
3. Place `annotator_1.csv`, `annotator_2.csv` and, after reviewing disagreement output, optional `adjudicator.csv` in one GitHub Actions artifact.
4. Dispatch `.github/workflows/ch1-evaluate-independent-detector-audit.yml` with that human-response artifact ID. The default packet, key and prediction IDs are the executed immutable artifacts listed above.

The evaluation workflow validates annotation completeness, refuses unresolved double labels, computes the frozen detector metrics and uploads the adjudicated ground truth, error queues, stratified summaries and workflow provenance.

Equivalent local commands remain available:
