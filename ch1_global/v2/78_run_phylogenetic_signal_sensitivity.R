...
safe_signal <- function(tree, x, run_test) {
  ...
  list(
    status = "success",
    K = as.numeric(k_fit$K),
    K_p_value = if (run_test && !is.null(k_fit$P)) as.numeric(k_fit$P) else NA_real_,
    lambda = as.numeric(lambda_fit$lambda),
    lambda_p_value = if (run_test && !is.null(lambda_fit$P)) as.numeric(lambda_fit$P) else NA_real_,
    error = ""
  )
}
...
add_fit <- function(endpoint, endpoint_row, tree, scenario, replicate, information_tier, allowed_keys, run_test) {
  ...
  if (is.null(prepared)) {
    rows[[length(rows) + 1L]] <<- data.frame(
      ...
      endpoint = endpoint,
      interpretation = endpoint_row$interpretation,
      scenario = scenario,
      replicate = replicate,
      information_tier = information_tier,
      n_species = 0L,
      K = NA_real_, K_p_value = NA_real_, lambda = NA_real_, lambda_p_value = NA_real_,
...
summary_rows <- lapply(split(random, random$endpoint), function(group) {
  data.frame(
    ...
    interpretation = group$interpretation[[1]],
    n_successful_random_trees = nrow(group),
    n_species_min = min(group$n_species),
    n_species_max = max(group$n_species),
    K_median = median(group$K, na.rm = TRUE),
    K_q025 = quantile_safe(group$K, 0.025),
    K_q975 = quantile_safe(group$K, 0.975),
    lambda_median = median(group$lambda, na.rm = TRUE),
    lambda_q025 = quantile_safe(group$lambda, 0.025),
    lambda_q975 = quantile_safe(group$lambda, 0.975),
    stringsAsFactors = FALSE
  )
})
...
report <- list(
  ...
  n_input_taxa = nrow(species),
  n_direct_backbone_taxa = length(direct_keys),
  n_endpoints = length(endpoints),
  n_random_trees = length(scenario2),
  min_species = min_species,
  K_permutation_replicates_for_deterministic_fits = k_nsim,
  n_total_fits = nrow(results),
  n_successful_fits = sum(results$status == "success"),
