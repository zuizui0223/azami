...
n_factors <- as.integer(getarg("n-factors","3")); min_species_n <- as.integer(getarg("min-species-n","5"))
max_edge <- as.numeric(getarg("mesh-max-edge-km","750")); cutoff <- as.numeric(getarg("mesh-cutoff-km","50")); offset <- as.numeric(getarg("mesh-offset-km","1000"))
...
spde <- inla.spde2.matern(mesh,alpha=2); A <- inla.spde.make.A(mesh,loc=loc)
...
cpp <- file.path("ch1_global","v2","tmb_multivariate_spatial.cpp"); TMB::compile(cpp,flags="-O2"); dyn.load(TMB::dynlib(sub("\\.cpp$","",cpp)))
...
obj <- MakeADFun(data=list(y=as.numeric(y),obs_index=obs_index,trait_index=trait_index,species_index=as.integer(species_index),X=X,A=as(A,"dgTMatrix"),M0=as(spde$param.inla$M0,"dgTMatrix"),M1=as(spde$param.inla$M1,"dgTMatrix"),M2=as(spde$param.inla$M2,"dgTMatrix"),n_traits=n_traits,n_factors=n_factors),parameters=pars,random=c("species_re","omega"),map=list(loadings=lm),DLL="tmb_multivariate_spatial",silent=TRUE)
opt <- nlminb(obj$par,obj$fn,obj$gr,control=list(iter.max=1000,eval.max=2000)); fit <- sdreport(obj,par.fixed=opt$par,getJointPrecision=FALSE); ph <- obj$env$parList(opt$par)
