...
1. YOLO-detected head metadata and image URLs;
2. reconstructed head/context crops;
3. accepted taxon and observation metadata;
4. CHELSA observation-level climate values.

Record the SHA-256 checksum of every input table or input artifact together with
the Git commit. Do not use a mutable Actions artifact without recording its
identifier and downloaded ZIP checksum.

## 2. Continuous primary traits

```bash
python ch1_global/v2/56_run_primary_traits_continuous_v2.py \
  --packet-manifest INPUT/continuous_trait_manifest.csv \
...
python ch1_global/v2/53_aggregate_primary_trait_continuous.py \
  --head-measurements work/continuous_head/primary_trait_continuous_head_measurements.csv \
  --head-metadata INPUT/ai_trait_head_level_with_metadata.csv \
  --out-dir work/continuous_aggregated \
  --min-heads-per-observation 1 \
  --min-observations-per-species 3
```
...
The QC GEE models diagnose environmentally selective assessability. They are not
trait–environment effect models.
...
Unknown values remain unknown until a reviewed source, page/table/figure and
supporting quotation are present.

## 6. Integrate primary and auxiliary endpoints
...
python ch1_global/v2/62_join_primary_auxiliary_continuous.py \
  ...
  --auxiliary-observation work/involucre/measurements/involucre_auxiliary_observation_level.csv \
  --auxiliary-species work/involucre/measurements/involucre_auxiliary_species_level.csv \
...
python ch1_global/v2/64_build_prephylogenetic_effect_figures.py \
  --coefficients work/prephylogenetic_models/continuous_environment_model_coefficients.csv \
  --out-dir work/prephylogenetic_figures
```
...
Pagel lambda is bounded to [0, 1]. A random-tree comparison is complete only
when all 50 replicates succeed.

## 8. Assemble the submission bundle

The validator requires one unambiguous final copy of each named result. Keep the
raw model report from `historical_models`, but use the cleaned coefficient and
random-tree tables from `historical_summary`.

```bash
rm -rf submission_bundle
...
cp -r work/prephylogenetic_figures submission_bundle/
cp -r work/historical_trees submission_bundle/
mkdir -p submission_bundle/historical_models
cp work/historical_models/historical_constraint_model_report.json \
  submission_bundle/historical_models/
...
Do not include model weights, temporary image packets, caches or two files with
the same required filename.
...
frozen without a passing validation report and checksummed manifest.
...
3. a new archive version rather than overwriting the submitted result.
