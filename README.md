...
All catalogued GitHub Actions source artifacts now also have durable off-Actions copies in the owner-controlled Google Drive archive created on 2026-09-04. Exact Drive file IDs, source SHA-256 values, chunk SHA-256 values and reassembly instructions for the 923 MB exhaustive checkpoint and 140 MB independent-audit annotation packet are recorded in `reproducibility/durable_archive_manifest.json`. GitHub Actions retention deadlines are therefore provenance only, not a remaining reproducibility dependency.
...
- `reproducibility/recovery_inventory.json` — active-v2 completeness, historical recovery locations and durability state.
- `reproducibility/durable_archive_manifest.json` — durable off-Actions source archive identities and reassembly contract.
