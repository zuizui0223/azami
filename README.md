...
The current Chapter 1 evidence chain is the following. This is the analysis path used by the paper; superseded categorical head-direction classifiers are historical development only and are not manuscript evidence.
...
within-taxon / among-taxon environmental analyses and whole-capitulum synthesis
  ...
  artifact 9632715852
  ↓
biological-construct reaggregation and robustness analysis
  workflow run 34418904597
  artifact 10130210432
```

The detector is used only to localize visible capitula. The current trait evidence is not generated by the retired MobileNet `upward`/`nodding` classifier. Continuous colour, orientation and morphology endpoints are measured from detector-defined image regions by the frozen v2 measurement implementation. The active implementation registry is [`ch1_global/v2/ANALYSIS_MANIFEST.tsv`](ch1_global/v2/ANALYSIS_MANIFEST.tsv), and the frozen Actions/artifact identities are recorded in [`reproducibility/actions_artifact_catalog.json`](reproducibility/actions_artifact_catalog.json).

The pre-cleanup scientific tree, including retired workflow definitions needed only for provenance, remains recoverable from immutable tag `azami-ch1-v2-2026-08-27` and branch `archive/ch1-precleanup-20260827`. Retired detector-bootstrap, categorical AI/classifier, queue-building and exploratory submission workflows must not be restored to `main` unless they become part of a new explicitly versioned analysis.

## Active GitHub Actions boundary

`main` intentionally contains only two workflow definitions:

- [`.github/workflows/reproducibility-integrity.yml`](.github/workflows/reproducibility-integrity.yml) — checks the public reproducibility layout, frozen contracts, provenance and live entry points on pushes and pull requests.
- [`.github/workflows/rebuild-frozen-figures.yml`](.github/workflows/rebuild-frozen-figures.yml) — rebuilds the frozen figure set in scratch space and verifies that the complete figure set can be regenerated.

The large image-processing and model/trait-generation runs listed above are frozen provenance, not mutable workflows on the current submission surface. Their source artifacts are durably archived and their historical workflow definitions remain available through the archive branch.
